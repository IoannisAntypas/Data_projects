...
//whenever slider is used, this function is called
//loading csv file and running functions to update the map with the data
//must be inside the csv-function, else old data might be used for the map
function load_data(this_filename) {
     ...
     d3.csv(this_filename, function(data) {
        ...
        //updates the color, but doesn't draw the polygons again
        update_color(data);
...
function update_color(district_data) {
  // select the svg with the id 'map' defined in attributes
    var svg = d3.select("#map");
    this_path = svg.selectAll("path");
    // 'g' is a function to group different objects
    // append g means you add a new group of objects
    var g = svg.append("g");
...
function add_information(district_data) {
    ...
    //on mouseover over a polygon run a function
...
// draw initial version of the map, using geojson
function draw_map(map_file) {

var width = 1000,
    height = 500;

//delete any old svg file on the html
d3.selectAll("svg").remove();

//draw a new svg-canvas, call it svg so it can be used later to add objects
//give the map the id 'map', because I have two svgs and I want to distinguish them
//select('em') means only find html-tag '<em>' and draw the svg there
//normally it's enough to write select('body')
var svg = d3.select("em").append("svg")
    .attr("id","map")
    .attr("width", width)
    .attr("height", height);

// needed to draw polygons later on
  var projection = d3.geoAlbers();
  var path = d3.geoPath();

// just a pattern to be used for the map (instead of color)
svg.append('defs')
.append("pattern")
.attr('id','mypattern')
.attr('width','8')
...
// load the geojson file and draw the map
// mapfile is the variable with the name of the json filename 
d3.json(map_file, function(error, mapData) {

  //select all path-objects (lines, used with polygons)
  //use the data from the geojson (mapData)
  //append path means add new paths to the svg
  //attributes and style and 'each' is all concerned with the new paths
  //'d' attribute says how to draw the paths defined above with projection (geopath and geoAlbers)
  //with 'each' you can call a function with the data of the geojson
  svg.selectAll('path').data(mapData.features).enter().append('path')
    ...
    // draws a rectangle as a frame for the legend
    svg.append("rect")
      .attr("x", "120")
      .attr("y", height-120)
      .attr('width','100')
...
function naming(d) {
    ...
    //check for San Francisco
    if(this_centroid[1]>0) {
    //write on the svg text with a x and y coordinate
    //we give this object the id 'districts' so we can refer to it later
    svg.append('text')
      .attr("id","districts")
      .attr("x",this_centroid[0]-20)
      .attr("y",this_centroid[1])
